...
continuous.data <- read.csv(file="continuous_data.csv", row.names = 1, stringsAsFactors = FALSE)
...
BM1 <- geiger::fitContinuous(tree, continuous.data, model = "BM")
print(paste("The rate of evolution is", "0.136160", "in units of", "meters"))
#The rate of evolution is 0.136160 in units of meters


OU1 <- fitContinuous(tree, continuous.data, model="OU")
par(mfcol(c(1,2)))
...
pp.OUwie <- OUwie(phy = tree, data = continuous.data, model = "OUM", algorithm = "invert", quiet = TRUE)
